...
"""
    ...
    try:
        ...
        status_label.config(text="qpAdm completed.")
        status_label.update()
...
""".strip()
    ...
    edit_run_button.grid(row=6, column=2, pady=10, sticky='we')
...
"""
        ...
        try:
            ...
            # Display results in the same format as run_qpadm()
            if weights_output:
                output_text.insert(tk.END, "Weights:\n" + weights_output + "\n")
            if popdrop_output:
                output_text.insert(tk.END, "Popdrop:\n" + popdrop_output + "\n")
...
left_entry = tk.Entry(scrollable_frame, width=60, textvariable=left_var)
left_entry.grid(row=1, column=1, padx=5, pady=2, sticky='we')
...
tk.Button(scrollable_frame, text="🡕", width=3, command=open_left_editor).grid(row=1, column=2, padx=(0, 5))


tk.Label(scrollable_frame, text="Right populations:").grid(row=2, column=0, sticky='w', padx=5, pady=2)
right_var = tk.StringVar()
right_entry = tk.Entry(scrollable_frame, width=60, textvariable=right_var)
right_entry.grid(row=2, column=1, padx=5, pady=2, sticky='we')
...
tk.Button(scrollable_frame, text="🡕", width=3, command=open_right_editor).grid(row=2, column=2, padx=(0, 5))
...
def setup_output_search():
    ...
    output_search_frame.grid(row=6, column=0, sticky='we', padx=5, pady=2)
...
rotation_frame.grid(row=9, column=0, columnspan=3, sticky='we', padx=5, pady=10)

tk.Label(rotation_frame, text="Fixed References (Right):").grid(row=0, column=0, sticky='w', padx=5)
fixed_right_entry = tk.Entry(rotation_frame, width=150)
...
fixed_left_entry = tk.Entry(rotation_frame, width=150)
...
rotation_pool_entry = tk.Entry(rotation_frame, width=150)
...
run_button.grid(row=6, column=1, pady=10, sticky='we')
...
rotation_run_button.grid(row=10, column=0, columnspan=3, pady=10, sticky='we')

#Button
save_output_button = tk.Button(scrollable_frame, text="💾 Save", command=save_output_to_file, bg="lightgray", width=10, height=1)
save_output_button.grid(row=11, column=2, sticky='e', padx=10, pady=(0, 10))

# --- OUTPUT CONSOLE ---
output_text = scrolledtext.ScrolledText(scrollable_frame, width=120, height=30)
output_text.grid(row=7, column=0, columnspan=3, padx=10, pady=5, sticky='nsew')

scrollable_frame.grid_rowconfigure(7, weight=1)

# Status label
status_label = tk.Label(scrollable_frame, text="Ready.", anchor='w')
status_label.grid(row=8, column=0, columnspan=3, sticky='we', padx=10, pady=(0, 10))
