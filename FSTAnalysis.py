...
pop2_entry = tk.Entry(scrollable_frame, width=70)
...
tk.Button(editor_controls, text="Undo", command=undo_ind_changes).pack(side=tk.LEFT)
tk.Button(editor_controls, text="Redo", command=redo_ind_changes).pack(side=tk.LEFT)
tk.Button(editor_controls, text="Save", command=save_ind_file).pack(side=tk.LEFT)
...
tk.Button(button_frame, text="Undo Add", command=undo_population_addition).pack(side=tk.LEFT, padx=5, expand=True)
tk.Button(button_frame, text="Redo Add", command=redo_population_addition).pack(side=tk.LEFT, padx=5, expand=True)
...
status_label.grid(row=9, column=0, columnspan=3, sticky='we', padx=10, pady=(0, 5))  # Moved up one row
...
def setup_output_search():
    ...
    output_search_frame.grid(row=8, column=0, columnspan=3, sticky='we', padx=5, pady=(0, 10))
    ...
    save_output_button = tk.Button(output_search_frame, text="💾 Save Output", 
                                 command=save_output_to_file, bg="lightgray")
    save_output_button.pack(side=tk.RIGHT, padx=(10, 0))
